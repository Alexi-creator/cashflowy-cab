...
import type { CategorySlice } from "../../lib/helpers"
          ...
          <Box ref={wrapRef} style={{ position: "relative" }}>
            ...
            >
              {slices.map((s) => {
                ...
                offset += s.pct
                return (
                  // biome-ignore lint/a11y/noStaticElementInteractions: hover tooltip on a decorative chart slice
                  <circle
                    key={s.id}
                    cx={CX}
                    cy={CY}
                    ...
                    onMouseMove={(e) => show(e, `${s.name}: ${money(s.total)} (${s.pct}%)`)}
                    onMouseLeave={hide}
                  />
                )
              })}
